...
/** All 140 CSS named colours with their sRGB values. */
const WEB_COLORS: { name: string; r: number; g: number; b: number }[] = [
...
/** Return the CSS named colour whose RGB is closest (Euclidean) to `c`. */
export function nearestWebColor(c: Color): { name: string; hex: string } {
  let best = WEB_COLORS[0];
  let bestDist = Infinity;

  for (const wc of WEB_COLORS) {
    const dr = wc.r - c.r, dg = wc.g - c.g, db = wc.b - c.b;
  ...
  const hex =
    '#' +
    best.r.toString(16).padStart(2, '0') +
    best.g.toString(16).padStart(2, '0') +
    best.b.toString(16).padStart(2, '0');
